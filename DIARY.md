...
- Verificare lo stato attuale dei moduli Python (`modules/`, `config/scripts/`, `temp_tests/`) per identificare:
...
- **Infra & I/O (TENERE)**
- `modules/telegram_handler.py` – usato da `manual_sender.py` e `config/scripts/send_telegram_reports.py` per l’invio Telegram.
...
- `config/scripts/send_telegram_reports.py` – richiamato da `SV_Start.bat` per inviare report JSON già salvati a Telegram; strumento operativo non di core.
