...
### 🔌 Portfolio decision layer (multi-broker)
The portfolio stack manages multi-asset, multi-broker exposure and produces operational JSON for the dashboard and Telegram:

- **Inputs**: ENGINE indicators + BRAIN signals + current positions; target allocations come from `config/config.txt`.
- **Decision logic**: allocator → risk manager → executor → state builder, with recommended trades only when deviation from targets exceeds ±10% and the broker is enabled (Bybit/IG for automated orders; Directa/Trade Republic for notifications only).
- **Target allocation**: Cash 20%, Indici/Azioni/ETF (incl. oro via ETF) 40%, Bonds 20%, Crypto 20% (solo Bybit; parte usata per bot trading).
- **Broker policies**: Bybit (bot, 50% capitale), IG Italia (bot, 50% margine), Directa e Trade Republic (notifiche operative, nessun ordine automatico).
- **Outputs**: `config/portfolio_state.json` (stato completo + storico operazioni) e `config/portfolio_signals.json` (segnali operativi inviati anche a Telegram); history aggiuntiva in `reports/portfolio_history/` quando abilitata.
- **Module layout (planned)**: `modules/portfolio/allocator.py`, `risk_manager.py`, `executor.py`, `portfolio_state_builder.py` per separare allocazione, rischio, esecuzione e costruzione JSON.
