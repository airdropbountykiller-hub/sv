...
- Per `DIARY.md` usiamo un merge driver `ours` (vedi `.gitattributes`). Se Git lo segnala comunque in conflitto: registra il driver built-in una volta sola con `git config merge.ours.driver true`, poi forza la versione locale con `git checkout --ours DIARY.md` e marca il file come risolto con `git add DIARY.md`. In alternativa esegui `bash temp_tests/tools/resolve_diary_conflict.sh` per applicare questi passaggi automaticamente.
