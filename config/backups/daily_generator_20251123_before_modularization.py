...
class DailyContentGenerator:
    ...
    def run_engine_brain_heartbeat(self, now: Optional[datetime.datetime] = None) -> None:
        """Lightweight ENGINE+BRAIN heartbeat (v2).

        Designed to be called periodically (e.g. every 30 minutes) by the orchestrator.
        It now produces a richer snapshot that can be used by the dashboard and
        future analytics:
        ...
        try:
            ...
            # 2) ENGINE snapshot: key assets (BTC, SPX, EURUSD, GOLD USD/g)
            assets: Dict[str, Any] = {}
            ...
            # 3) BRAIN-lite: live evaluation of predictions
            try:
                raw_prediction_eval = self._evaluate_predictions_with_live_data(now) or {}
            except Exception:
                raw_prediction_eval = {}
            ...
            # 4) Regime + tomorrow bias via DailyRegimeManager (when available)
            regime_info: Dict[str, Any] = {}
            if REGIME_MANAGER_AVAILABLE:
                try:
                    manager = get_daily_regime_manager()
                    # Update with full-day sentiment tracking when available
                    ...
                    # Update with real accuracy if we have tracked predictions
                    if total_tracked > 0:
            ...
            if PORTFOLIO_MANAGER_AVAILABLE:
                try:
                    ...
                    risk_info = {
                        ...
                        # Placeholders for future intraday risk analytics (kept stable for future wiring)
                        'intraday': {
                            'pnl': None,
                            'pnl_pct': None,
                    ...
                    prediction_eval['risk'] = risk_info
            ...
            try:
                ...
                try:
                    ...
                    news_list = [
                        ...
                        and not self._is_low_impact_gadget_or_lifestyle(it.get('title', ''))
                    ]
                    if news_list:
                ...
                try:
                    ...
                    if DEPENDENCIES_AVAILABLE:
                        ...
                        if recent_tracked >= 5 and recent_acc > 0:
                            ...
                            # Limited live history: fall back to sentiment-aware neutral regime
                            try:
                                raw_sentiment = (news_data.get('sentiment', {}) or {}).get('sentiment', 'NEUTRAL') if isinstance(news_data, dict) else 'NEUTRAL'
                            except Exception:
                                raw_sentiment = 'NEUTRAL'
                            upper_sent = str(raw_sentiment).upper()
                            if 'NEGATIVE' in upper_sent:
                                regime_label = "CAUTIOUS-NEUTRAL"
                                conf_pct = 60
                                tone = "sentiment negative with limited live history"
                            elif 'POSITIVE' in upper_sent:
                                regime_label = "NEUTRAL-BULLISH"
                                conf_pct = 65
                                tone = "sentiment supportive but limited live history"
                            else:
                                regime_label = "NEUTRAL"
                                conf_pct = 62
                                tone = "limited live history"
                            msg2_parts.append(
                                f"{EMOJI['bullet']} *Current Regime*: {regime_label} {EMOJI['right_arrow']} ({conf_pct}% confidence, {tone})"
                            )
    ...
    def generate_evening_analysis(self) -> List[str]:
        ...
        try:
            ...
            try:
                ...
                # Use Regime Manager for consistent narrative (v1.5.0 Enhancement)
                if REGIME_MANAGER_AVAILABLE:
                    try:
                        manager = get_daily_regime_manager()
                        
                        # Update with current sentiment
                        session_sentiment = news_data.get('sentiment', {}).get('sentiment', 'NEUTRAL')
                        manager.update_from_sentiment_tracking({'evening': session_sentiment})
                        
                        # Update with any prediction accuracy data if available
                        try:
                            pred_eval = self._evaluate_predictions_with_live_data(now)
                            if pred_eval and pred_eval.get('total_tracked', 0) > 0:
                                accuracy_pct = pred_eval.get('accuracy_pct', 0.0)
                                total_tracked = pred_eval.get('total_tracked', 0)
                                manager.update_from_accuracy(accuracy_pct, total_tracked)
                        except Exception as eval_e:
                            log.warning(f"{EMOJI['warning']} [REGIME-UPDATE] Accuracy eval error: {eval_e}")
                        
                        # Generate consistent texts using Regime Manager
                        session_character = manager.get_session_character()
                        regime = manager.infer_regime()
                        
                        msg1_parts.append(f"{EMOJI['bullet']} *Theme*: {session_character}")
                        
                        # Volume and breadth based on regime
                        ...
                        log.info(f"[OK] [REGIME-MANAGER] Evening character: {session_character}")
                        ...
                        msg1_parts.append(f"{EMOJI['bullet']} *Volume*: Standard levels - see performance details")
                        msg1_parts.append(f"{EMOJI['bullet']} *Breadth*: Market breadth - see sector analysis")
                else:
...
# DETAILED PREDICTION BREAKDOWN
                ...
                try:
                    # Derive an evening accuracy snapshot when available
                    acc_evening_pct = None
                    if 'hits' in locals() and 'total' in locals() and isinstance(total, (int, float)) and total > 0:
                        try:
                            acc_evening_pct = (hits / total) * 100.0
                        except Exception:
                            acc_evening_pct = None
                    ...
                    if DEPENDENCIES_AVAILABLE:
                        ...
                        if acc_evening_pct is not None:
                            if acc_evening_pct >= 80:
                                msg2_parts.append(f"{EMOJI['bullet']} *Model Consensus*: High agreement with market moves (≈{acc_evening_pct:.0f}% accuracy)")
                                msg2_parts.append(f"{EMOJI['bullet']} *Model Calibration*: Well aligned with current regime")
                            elif acc_evening_pct >= 60:
                                msg2_parts.append(f"{EMOJI['bullet']} *Model Consensus*: Generally consistent with price action (≈{acc_evening_pct:.0f}% accuracy)")
                                msg2_parts.append(f"{EMOJI['bullet']} *Model Calibration*: Solid but with some noise")
                            elif acc_evening_pct > 0:
                                msg2_parts.append(f"{EMOJI['bullet']} *Model Consensus*: Mixed signals (≈{acc_evening_pct:.0f}% accuracy)")
                                msg2_parts.append(f"{EMOJI['bullet']} *Model Calibration*: Requires refinement for current regime")
                            else:
                                msg2_parts.append(f"{EMOJI['bullet']} *Model Consensus*: Signals not aligned with today’s price action (0% accuracy on tracked calls)")
                                msg2_parts.append(f"{EMOJI['bullet']} *Model Calibration*: Defensive review in progress – see Daily Summary Page 1/2")
                        else:
                            msg2_parts.append(f"{EMOJI['bullet']} *Model Consensus*: Evaluated using ensemble agreement and recent history")
                            msg2_parts.append(f"{EMOJI['bullet']} *Model Calibration*: See Daily Summary Page 1/2 for full accuracy details")
                        ...
                        if acc_evening_pct is not None:
                            ...
                            msg2_parts.append(f"{EMOJI['bullet']} *Model Consensus*: Evaluated qualitatively – no live-tracked predictions today")
    ...
    def generate_daily_summary(self) -> List[str]:
        ...
        try:
            ...
            if REGIME_MANAGER_AVAILABLE:
                try:
                    manager = get_daily_regime_manager()
                    # Normalizza sentiment_tracking in un semplice dizionario di stringhe
                    ...
                    if simple_sentiments:
                        manager.update_from_sentiment_tracking(simple_sentiments)
                    else:
            ...
            try:
                ...
                regime_str = unified_regime.value if unified_regime is not None else 'neutral'
            ...
            try:
                ...
                if total_for_model > 0:
                    ...
                    signal_quality = "Assessed qualitatively (no live-tracked predictions today)"
...
def run_engine_brain_heartbeat() -> None:
    ...
    This helper is used by the main orchestrator to log intraday ENGINE/BRAIN
    snapshots approximately every 30 minutes without generating any messages.
    """
    generator = get_daily_generator()
    generator.run_engine_brain_heartbeat()
