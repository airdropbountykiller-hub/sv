        ...
        try:
            ...
            if hasattr(module, 'main'):
                ...
                if success:
                    ...
                    # Central trader hook: after MORNING predictions are saved, run the
                    # backtest-only trader to open simulated portfolio positions.
                    if content_type == 'morning':
                        try:
                            from modules import portfolio_trader

                            result = portfolio_trader.run_daily_trader(simulate_only=True)
                            opened = result.get('opened_positions') if isinstance(result, dict) else None
                            opened_n = len(opened) if isinstance(opened, list) else 0
                            log.info(f"[TRADER] Completed (simulate_only). Opened={opened_n}")
                        except Exception as e:
                            log.warning(f"[TRADER] Error running daily trader: {e}")
                else:
