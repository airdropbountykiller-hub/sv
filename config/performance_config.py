...
PERFORMANCE_CONFIG = {
    ...
    "max_news_per_source": 5,     # Max 5 news per fonte RSS
    
    # Dataset ottimizzazioni aggressive
    "max_datapoints_ml": 500,     # Dimezzato: era 1000, ora 500 per velocitÃ 
...
LIGHTNING_ML_MODELS = [
    ...
    "Gradient Boosting",  # Buon compromesso velocitÃ /accuratezza
...
# === RSS FEEDS SUPER-OTTIMIZZATI (1 PER category) ===
ULTRA_FAST_RSS_FEEDS = {
    "Finanza": [
        "https://feeds.reuters.com/reuters/businessNews"  # Solo Reuters - il piÃ¹ veloce
    ],
    "Criptovalute": [
...
def timed_execution(func_name="Function"):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            end_time = time.time()
            print(f"âš¡ [{func_name}] Completato in {end_time - start_time:.2f}s")
            return result
        return wrapper
    return decorator
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if (key in cache and 
                ...
                print(f"ðŸš€ [CACHE-HIT] {func.__name__}")
                return cache[key]
            ...
            print(f"ðŸ’¾ [CACHE-MISS] {func.__name__} - Salvato in cache")
            return result
        return wrapper
    return decorator
...
def get_thread_pool():
    ...
    with _thread_pool_lock:
        if _thread_pool is None:
            ...
            print(f"ðŸ§µ [THREADING] Thread pool inizializzato con {PERFORMANCE_CONFIG['max_workers']} workers")
        return _thread_pool
...
def parallel_execute(functions_with_args):
    ...
    for name, future in futures:
        try:
            ...
            print(f"âŒ [PARALLEL] Errore in {name}: {e}")
            results[name] = None
...
DEBUG_PERFORMANCE = {
    ...
    "enable_memory_logs": False,   # Disabilita per velocitÃ  (attivare solo se necessario)
    "enable_cache_stats": True,    # Statistiche cache hit/miss
    "enable_parallel_logs": True,  # Log delle operazioni parallele
}

print("âœ… [PERFORMANCE] Configurazione caricata - ModalitÃ  velocitÃ  attiva!")
