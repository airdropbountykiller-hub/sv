...
try:
    # Optional dependency: only used when routing orders to Bybit
    from modules.api.brokers.bybit import BybitClient
except Exception:
    BybitClient = None  # type: ignore

try:
    # Optional dependency: only used when routing orders to IG
    from modules.api.brokers.ig import IGClient
except Exception:
    IGClient = None  # type: ignore

logger = logging.getLogger(__name__)

# Default toggles (overridable via env vars and/or function parameters)
...
class SVPortfolioManager:
    """Manages a $10K simulated portfolio tracking ML signals."""
    ...
    def __init__(
        ...
        *,
        ...
        if base_dir is None:
            base_dir = sv_paths.PROJECT_ROOT
        ...
        self.allow_manual_brokers = (
            allow_manual_brokers
            if allow_manual_brokers is not None
            else ALLOW_MANUAL_BROKERS_DEFAULT
        )
        ...
        # Diversification: cap max exposure per single asset across the whole portfolio
        # (e.g., avoid being all-in on BTC). Override via env var.
        cap_raw = os.getenv('SV_PORTFOLIO_MAX_ASSET_EXPOSURE_PCT', '0.35')
        try:
            self.max_asset_exposure_pct = float(cap_raw)
        except Exception:
            self.max_asset_exposure_pct = 0.35
        if self.max_asset_exposure_pct < 0:
            self.max_asset_exposure_pct = 0.0
        if self.max_asset_exposure_pct > 1:
            self.max_asset_exposure_pct = 1.0
    ...
    def _save_portfolio(self, portfolio: Dict[str, Any]):
        """Save portfolio state to disk (atomic write to avoid partial/corrupt JSON)."""
        try:
            portfolio_path = Path(self.portfolio_file)
            ...
            os.replace(str(tmp_path), str(portfolio_path))
        except Exception as e:
    ...
    def reset_portfolio(
        ...
        *,
        ...
        Args:
            ...
            backup_state: if True, archives the previous portfolio_state.json under
                reports/portfolio_backups before resetting.
        """

        if include_manual_brokers is not None:
            self.allow_manual_brokers = include_manual_brokers

        # Optional backup of existing state
        if backup_state:
            try:
                src = Path(self.portfolio_file)
                if src.exists():
                    backups_dir = Path(self.base_dir) / 'reports' / 'portfolio_backups'
                    backups_dir.mkdir(parents=True, exist_ok=True)
                    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
                    dst = backups_dir / f"portfolio_state_{ts}.json"
                    shutil.copy2(src, dst)
                    logger.info("Archived previous portfolio state to %s", dst)
            except Exception as exc:
                logger.warning("Could not archive previous portfolio state: %s", exc)
    ...
    def open_position(
        ...
        *,
        ...
        """Open new position based on a prediction.

        IMPORTANT: This function is safe-by-default. Unless simulate_only=False is
        explicitly provided, it will never route real orders to brokers.
        """
        try:
            ...
            # Broker routing:
            # - if caller doesn't specify broker, route crypto to Bybit USDT perps (testnet-first)
            broker = (prediction.get('broker') or '').strip()
            if not broker:
                cluster = self.asset_clusters.get(asset, 'other')
                broker = 'BYBIT_USDT' if cluster == 'crypto' else 'IG'
            ...
            # Global diversification cap (per asset across all brokers)
            if self.max_asset_exposure_pct > 0:
                current_asset_exposure = sum(
                    float(p.get('position_size', 0) or 0)
                    for p in self.portfolio.get('active_positions', [])
                    if (p.get('asset') or '') == asset
                )
                max_asset_amount = self.initial_capital * self.max_asset_exposure_pct
                if (current_asset_exposure + position_size) > max_asset_amount:
                    logger.info(
                        f"Global asset cap reached for {asset}: "
                        f"{current_asset_exposure + position_size:.2f} > {max_asset_amount:.2f} "
                        f"({self.max_asset_exposure_pct:.0%} of total). Skipping."
                    )
                    return None
            ...
            # Optional: execute on broker (real order) when enabled.
            ...
            # Enforce intended routing even in simulation: we do NOT trade crypto on IG.
            if broker == 'IG':
                cluster = self.asset_clusters.get(asset, 'other')
                if cluster == 'crypto':
                    logger.info(f"Refusing crypto execution on IG for {asset}; use Bybit brokers")
                    return None

            # Parse/record leverage for Bybit brokers even in simulation (for dashboard display)
            if broker == 'BYBIT_USDT':
                leverage_raw = prediction.get('leverage')
                if leverage_raw is None or leverage_raw == '':
                    leverage_raw = get_private_value('BYBIT_DEFAULT_LEVERAGE', '3')
                try:
                    leverage_val = int(float(leverage_raw))
                except Exception:
                    leverage_val = 3
                if leverage_val < 0:
                    leverage_val = 0

            # Real broker execution is explicitly opt-in.
            if not simulate_only:
                # IG demo execution (indices/fx/commodities).
                if broker == 'IG':
                    if IGClient is not None:
                        ig = IGClient()
                        if ig.is_configured() and ig.enable_trading:
                            # Enforce demo-only in this setup
                            if ig.env != 'demo':
                                logger.info('IG trading is restricted to DEMO in this setup; skipping')
                                return None
                            epic_map = {
                                'SPX': get_private_value('IG_EPIC_SPX', ''),
                                'EURUSD': get_private_value('IG_EPIC_EURUSD', ''),
                                'GOLD': get_private_value('IG_EPIC_GOLD', ''),
                            }
                            epic = epic_map.get(asset.upper(), '')
                            if not epic:
                                logger.info(f"IG epic missing for asset {asset}; skipping")
                                return None

                            # IG deal size is instrument-specific; require explicit configuration.
                            size_key = f"IG_DEAL_SIZE_{asset.upper()}"
                            size_raw = (prediction.get('ig_deal_size') or get_private_value(size_key, '') or '').strip()
                            if not size_raw:
                                logger.info(f"Missing {size_key} (or prediction ig_deal_size). Refusing IG order for safety.")
                                return None
                            try:
                                desired_size = float(size_raw)
                            except Exception:
                                logger.info(f"Invalid {size_key}={size_raw!r}. Refusing IG order for safety.")
                                return None
                            ...
                            order = ig.place_order(
                                symbol=epic,
                                side=side,
                                quantity=desired_size,
                                order_type='market',
                                price=None,
                                reduce_only=False,
                                metadata={
                                    'stopLevel': stop_price,
                                    'limitLevel': target_price,
                                    'currencyCode': 'USD',
                                },
                            )
                            ...
                            execution['note'] = 'IG not enabled/configured'
                    else:
                        execution['note'] = 'IG client unavailable'

                if broker == 'BYBIT_USDT':
                    if BybitClient is not None:
                        bybit = BybitClient()
                        if bybit.is_configured() and bybit.enable_trading:
                            # Enforce testnet-only in this setup
                            if not bybit.testnet:
                                logger.info('Bybit trading is restricted to TESTNET in this setup; skipping')
                                return None
                            # Map asset -> USDT-linear perp symbol (simple default)
                            symbol = f"{asset.upper()}USDT"

                            # Fetch lot size filter to quantize qty
                            info = bybit.get_instruments_info(category='linear', symbol=symbol) or {}
                            lst = ((info.get('result') or {}).get('list') if isinstance(info, dict) else None) or []
                            item = lst[0] if isinstance(lst, list) and lst else {}
                            lot = (item.get('lotSizeFilter') if isinstance(item, dict) else None) or {}

                            qty_step = float(lot.get('qtyStep') or 0) if isinstance(lot, dict) else 0.0
                            min_qty = float(lot.get('minOrderQty') or 0) if isinstance(lot, dict) else 0.0
                            max_qty = float(lot.get('maxOrderQty') or 0) if isinstance(lot, dict) else 0.0

                            raw_qty = float(units)
                            if qty_step and qty_step > 0:
                                q = Decimal(str(raw_qty))
                                step = Decimal(str(qty_step))
                                qty = float((q / step).to_integral_value(rounding=ROUND_DOWN) * step)
                            else:
                                qty = raw_qty

                            if min_qty and qty < min_qty:
                                logger.info(f"Bybit qty below minOrderQty ({qty} < {min_qty}) for {symbol}; skipping")
                                return None
                            ...
                            side = 'Buy' if direction.upper() == 'LONG' else 'Sell'
                            if leverage_val > 0:
                                bybit.set_leverage(category='linear', symbol=symbol, leverage=leverage_val)
                            ...
                            execution['note'] = 'Bybit not enabled/configured'
                    else:
                        execution['note'] = 'Bybit client unavailable'

                elif broker == 'BYBIT_BTC':
                    # Not implemented yet (inverse contracts differ in sizing/symbol semantics)
                    execution['note'] = 'BYBIT_BTC execution not implemented yet'
            else:
                # Backtest-only mode: no side effects.
                execution['note'] = 'simulate_only'
    ...
    def update_positions(self, live_prices: Dict[str, Dict[str, float]]):
        ...
        for position in self.portfolio['active_positions']:
            ...
            # Check for stop/target hits (skip when broker holds native stop/limit orders)
            exec_meta = position.get('execution', {}) or {}
            if not exec_meta.get('managed_by_broker', False):
                if direction.upper() == 'LONG':
