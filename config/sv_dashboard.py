...
@app.route('/api/trader_status')
def trader_status():
    """Expose the central trader status (idempotency state + last run summary).

    Reads config/portfolio_signals.json (sv_paths.PORTFOLIO_SIGNALS_FILE).
    Never raises; returns an empty structure on missing/unreadable file.
    """
    try:
        status_path = sv_paths.PORTFOLIO_SIGNALS_FILE
        ...
        return jsonify({
            'ok': False,
            'path': sv_paths.PORTFOLIO_SIGNALS_FILE,
            'data': {},
            'error': str(e),
            'timestamp': datetime.now().isoformat(),
        })
