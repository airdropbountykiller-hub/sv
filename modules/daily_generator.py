...
Generates the SV daily cycle content (00/03/06/09/12/15/18/21) based on 555a_server approach
"""
...
def get_live_equity_fx_quotes(symbols: list[str]) -> dict:
    ...
    Delegates to `modules.api.market_data.get_live_equity_fx_quotes` so we can
    reuse caching/backoff and support provider fallback (Yahoo -> IG).
    """
    try:
        from modules.api.market_data import get_live_equity_fx_quotes as api_get_quotes

        return api_get_quotes(symbols) or {}
    except Exception as e:
...
class DailyContentGenerator:
    ...
    def _verify_full_day_coherence(self, now):
        """Verify coherence across the full 8-checkpoint daily cycle.

        Expected schedule (Italy time): 00:00 → 03:00 → 06:00 → 09:00 → 12:00 → 15:00 → 18:00 → 21:00.

        Notes:
        - Daily artifacts are saved under reports/8_daily_content as:
          YYYY-MM-DD_HHMM_<content_type>.json
        - Some saved content_type identifiers differ from scheduler content labels
          (e.g. morning_report vs morning). This function maps stages to saved IDs.
        """
        coherence_results = {
            'night_late_night_coherence': 0.0,
            'late_night_press_review_coherence': 0.0,
            'press_review_morning_coherence': 0.0,
            'morning_noon_coherence': 0.0,
            'noon_afternoon_coherence': 0.0,
            'afternoon_evening_coherence': 0.0,
        ...
        try:
            ...
            def _load_latest_saved(date_ymd: str, saved_content_type: str):
                """Load the latest saved artifact for a given date+content_type."""
                try:
                    pattern = f"{date_ymd}_*_{saved_content_type}.json"
                    matches = sorted(self.reports_dir.glob(pattern))
                    if not matches:
                        return None
                    latest = matches[-1]
            ...
            # Map stage -> saved content_type identifier used by save_content()
            stage_to_saved_type = {
                'night': 'night_report',
                'late_night': 'late_night_report',
                'press_review': 'press_review',
                'morning': 'morning_report',
                'noon': 'noon_update',
                'afternoon': 'afternoon_update',
                'evening': 'evening_analysis',
                'summary': 'daily_summary',
            }

            daily_contents = {}
            for stage, saved_type in stage_to_saved_type.items():
                loaded = _load_latest_saved(date_str, saved_type)
                if loaded is not None:
                    daily_contents[stage] = loaded

            # Progressive coherence checks across adjacent checkpoints
            pairs = [
                ('night', 'late_night', 'night_late_night_coherence'),
                ('late_night', 'press_review', 'late_night_press_review_coherence'),
                ('press_review', 'morning', 'press_review_morning_coherence'),
                ('morning', 'noon', 'morning_noon_coherence'),
                ('noon', 'afternoon', 'noon_afternoon_coherence'),
                ('afternoon', 'evening', 'afternoon_evening_coherence'),
                ('evening', 'summary', 'evening_summary_coherence'),
            ]

            for a, b, key in pairs:
                if a in daily_contents and b in daily_contents:
                    coherence_results[key] = self._calculate_content_coherence(
                        daily_contents[a], daily_contents[b]
                    )

            # Calculate overall coherence (average of available pair scores)
            ...
            # Basic inconsistency flags
            if not daily_contents:
                coherence_results['inconsistencies'].append('No saved intraday artifacts found for today')
            elif coherence_results['overall_coherence'] < 0.7:
                coherence_results['inconsistencies'].append('Low overall coherence between checkpoints')

        except Exception as e:
            cross = EMOJI.get('cross', '❌') if isinstance(EMOJI, dict) else '❌'
            log.error(f"{cross} [COHERENCE-CHECK] Error: {e}")

        return coherence_results

    def _calculate_content_coherence(self, content1, content2):
        """Calcola coerenza tra due contenuti usando analisi testuale semplice."""
        try:
            ...
            if not words1 or not words2:
                return 0.5  # Coerenza neutra se non c'è testo
            ...
            warn = EMOJI.get('warning', '⚠️') if isinstance(EMOJI, dict) else '⚠️'
            log.warning(f"{warn} [COHERENCE-CALC] Error: {e}")
            return 0.5

    def _extract_text_from_content(self, content_data):
        """Estrae testo da struttura dati content.

        Supports SV save_content() format (dict with 'messages') and older structures.
        """
        try:
            if isinstance(content_data, dict):
                # Current save_content() payload
    ...
    def generate_press_review(self) -> list[str]:
        """Generate the 06:00 Press Review (7 messages)."""
        from modules.generators.press_review import generate_press_review as _impl
        return _impl(self)
...
'Geopolitics': [
    ...
    def generate_night_report(self) -> List[str]:
        """Generate the 00:00 Night Report (1 message)."""
        from modules.generators.night import generate_night_report as _impl
        return _impl(self)

    def generate_late_night_report(self) -> List[str]:
        """Generate the 03:00 Late Night Update (1 message)."""
        from modules.generators.late_night import generate_late_night_report as _impl
        return _impl(self)

    def generate_morning_report(self) -> List[str]:
        """Generate the 09:00 Morning Report (3 messages)."""
        from modules.generators.morning import generate_morning_report as _impl
        return _impl(self)

    def generate_noon_update(self) -> List[str]:
        """Generate the 12:00 Noon Update (3 messages)."""
        from modules.generators.noon import generate_noon_update as _impl
        return _impl(self)

    def generate_afternoon_update(self) -> List[str]:
        """Generate the 15:00 Afternoon Update (3 messages)."""
        from modules.generators.afternoon import generate_afternoon_update as _impl
        return _impl(self)

    def generate_evening_analysis(self) -> List[str]:
        """Generate the 18:00 Evening Analysis (3 messages)."""
        from modules.generators.evening import generate_evening_analysis as _impl
        return _impl(self)

    def generate_daily_summary(self) -> List[str]:
        """Generate the 21:00 Daily Summary (6 pages)."""
...
def generate_night() -> List[str]:
    """Generate night report (1 message)"""
    generator = get_daily_generator()
    return generator.generate_night_report()


def generate_late_night() -> List[str]:
    """Generate late night update (1 message)"""
    generator = get_daily_generator()
    return generator.generate_late_night_report()
...
def generate_afternoon() -> List[str]:
    """Generate afternoon update (3 messages)"""
    generator = get_daily_generator()
    return generator.generate_afternoon_update()
...
def generate_summary() -> List[str]:
    """Generate daily summary (6 pages as separate messages)"""
    generator = get_daily_generator()
