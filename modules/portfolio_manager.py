...
def _ensure_manual_defaults() -> None:
    """Recreate manual broker defaults if globals were cleared or reloaded."""

    global ALLOW_MANUAL_BROKERS_DEFAULT, allow_manual_brokers

    current_default = globals().get("ALLOW_MANUAL_BROKERS_DEFAULT", None)
    if current_default is None:
        ALLOW_MANUAL_BROKERS_DEFAULT = os.getenv("SV_PORTFOLIO_ALLOW_MANUAL_BROKERS", "1") == "1"
    else:
        ALLOW_MANUAL_BROKERS_DEFAULT = current_default

    current_toggle = globals().get("allow_manual_brokers", None)
    if current_toggle is None:
        allow_manual_brokers = ALLOW_MANUAL_BROKERS_DEFAULT
    else:
        allow_manual_brokers = current_toggle
